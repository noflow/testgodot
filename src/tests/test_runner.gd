...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 110, "Registry indexes 110 production VN backgrounds across twenty completed locations.")
	...
	_expect(artwork_phases.size() == 14 and artwork_asset_count == 110, "VN art provides fourteen prioritized production phases covering 110 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 20, "Room-first production records twenty fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 110, "Twenty complete locations provide 110 unique ready base-day backgrounds.")
