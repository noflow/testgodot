...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 179, "Registry indexes 179 production VN backgrounds across thirty-three completed locations.")
	...
	_expect(artwork_phases.size() == 27 and artwork_asset_count == 179, "VN art provides twenty-seven prioritized production phases covering 179 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 33, "Room-first production records thirty-three fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 179, "Thirty-three complete locations provide 179 unique ready base-day backgrounds.")
