...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 60, "Registry indexes 60 production VN backgrounds across twelve completed locations.")
	...
	_expect(artwork_phases.size() == 6 and artwork_asset_count == 60, "VN art provides six prioritized production phases covering 60 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 12, "Room-first production records twelve fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 60, "Twelve complete locations provide 60 unique ready base-day backgrounds.")
