...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 155, "Registry indexes 155 production VN backgrounds across twenty-nine completed locations.")
	...
	_expect(artwork_phases.size() == 23 and artwork_asset_count == 155, "VN art provides twenty-three prioritized production phases covering 155 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 29, "Room-first production records twenty-nine fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 155, "Twenty-nine complete locations provide 155 unique ready base-day backgrounds.")
