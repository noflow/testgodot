...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 321, "Registry indexes 321 production VN backgrounds across fifty-seven completed locations.")
	...
	_expect(artwork_phases.size() == 51 and artwork_asset_count == 321, "VN art provides fifty-one prioritized production phases covering 321 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 57, "Room-first production records fifty-seven fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 321, "Fifty-seven complete locations provide 321 unique ready base-day backgrounds.")
