...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 106, "Registry indexes 106 production VN backgrounds across nineteen completed locations.")
	...
	_expect(artwork_phases.size() == 13 and artwork_asset_count == 106, "VN art provides thirteen prioritized production phases covering 106 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 19, "Room-first production records nineteen fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 106, "Nineteen complete locations provide 106 unique ready base-day backgrounds.")
