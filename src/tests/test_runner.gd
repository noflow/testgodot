...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 279, "Registry indexes 279 production VN backgrounds across forty-nine completed locations.")
	...
	_expect(artwork_phases.size() == 43 and artwork_asset_count == 279, "VN art provides forty-three prioritized production phases covering 279 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 49, "Room-first production records forty-nine fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 279, "Forty-nine complete locations provide 279 unique ready base-day backgrounds.")
