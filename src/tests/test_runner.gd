...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 336, "Registry indexes 336 production VN backgrounds across sixty completed locations.")
	...
	_expect(artwork_phases.size() == 54 and artwork_asset_count == 336, "VN art provides fifty-four prioritized production phases covering 336 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 60, "Room-first production records sixty fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 336, "Sixty complete locations provide 336 unique ready base-day backgrounds.")
