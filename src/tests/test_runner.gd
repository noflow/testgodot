...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 254, "Registry indexes 254 production VN backgrounds across forty-four completed locations.")
	...
	_expect(artwork_phases.size() == 38 and artwork_asset_count == 254, "VN art provides thirty-eight prioritized production phases covering 254 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 44, "Room-first production records forty-four fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 254, "Forty-four complete locations provide 254 unique ready base-day backgrounds.")
