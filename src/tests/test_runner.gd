...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 268, "Registry indexes 268 production VN backgrounds across forty-seven completed locations.")
	...
	_expect(artwork_phases.size() == 41 and artwork_asset_count == 268, "VN art provides forty-one prioritized production phases covering 268 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 47, "Room-first production records forty-seven fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 268, "Forty-seven complete locations provide 268 unique ready base-day backgrounds.")
