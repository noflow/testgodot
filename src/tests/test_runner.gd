...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 161, "Registry indexes 161 production VN backgrounds across thirty completed locations.")
	...
	_expect(artwork_phases.size() == 24 and artwork_asset_count == 161, "VN art provides twenty-four prioritized production phases covering 161 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 30, "Room-first production records thirty fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 161, "Thirty complete locations provide 161 unique ready base-day backgrounds.")
