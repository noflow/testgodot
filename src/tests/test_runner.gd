...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 282, "Registry indexes 282 production VN backgrounds across fifty completed locations.")
	...
	_expect(artwork_phases.size() == 44 and artwork_asset_count == 282, "VN art provides forty-four prioritized production phases covering 282 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 50, "Room-first production records fifty fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 282, "Fifty complete locations provide 282 unique ready base-day backgrounds.")
