...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 173, "Registry indexes 173 production VN backgrounds across thirty-two completed locations.")
	...
	_expect(artwork_phases.size() == 26 and artwork_asset_count == 173, "VN art provides twenty-six prioritized production phases covering 173 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 32, "Room-first production records thirty-two fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 173, "Thirty-two complete locations provide 173 unique ready base-day backgrounds.")
