...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 264, "Registry indexes 264 production VN backgrounds across forty-six completed locations.")
	...
	_expect(artwork_phases.size() == 40 and artwork_asset_count == 264, "VN art provides forty prioritized production phases covering 264 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 46, "Room-first production records forty-six fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 264, "Forty-six complete locations provide 264 unique ready base-day backgrounds.")
