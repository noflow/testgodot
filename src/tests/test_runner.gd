...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 167, "Registry indexes 167 production VN backgrounds across thirty-one completed locations.")
	...
	_expect(artwork_phases.size() == 25 and artwork_asset_count == 167, "VN art provides twenty-five prioritized production phases covering 167 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 31, "Room-first production records thirty-one fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 167, "Thirty-one complete locations provide 167 unique ready base-day backgrounds.")
