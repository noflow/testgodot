...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 312, "Registry indexes 312 production VN backgrounds across fifty-five completed locations.")
	...
	_expect(artwork_phases.size() == 49 and artwork_asset_count == 312, "VN art provides forty-nine prioritized production phases covering 312 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 55, "Room-first production records fifty-five fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 312, "Fifty-five complete locations provide 312 unique ready base-day backgrounds.")
