...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 198, "Registry indexes 198 production VN backgrounds across thirty-six completed locations.")
	...
	_expect(artwork_phases.size() == 30 and artwork_asset_count == 198, "VN art provides thirty prioritized production phases covering 198 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 36, "Room-first production records thirty-six fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 198, "Thirty-six complete locations provide 198 unique ready base-day backgrounds.")
