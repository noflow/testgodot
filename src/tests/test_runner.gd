...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 290, "Registry indexes 290 production VN backgrounds across fifty-two completed locations.")
	...
	_expect(artwork_phases.size() == 46 and artwork_asset_count == 290, "VN art provides forty-six prioritized production phases covering 290 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 52, "Room-first production records fifty-two fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 290, "Fifty-two complete locations provide 290 unique ready base-day backgrounds.")
