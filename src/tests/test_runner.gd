...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 124, "Registry indexes 124 production VN backgrounds across twenty-two completed locations.")
	...
	_expect(artwork_phases.size() == 16 and artwork_asset_count == 124, "VN art provides sixteen prioritized production phases covering 124 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 22, "Room-first production records twenty-two fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 124, "Twenty-two complete locations provide 124 unique ready base-day backgrounds.")
