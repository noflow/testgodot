...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 103, "Registry indexes 103 production VN backgrounds across eighteen completed locations.")
	...
	_expect(artwork_phases.size() == 12 and artwork_asset_count == 103, "VN art provides twelve prioritized production phases covering 103 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 18, "Room-first production records eighteen fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 103, "Eighteen complete locations provide 103 unique ready base-day backgrounds.")
