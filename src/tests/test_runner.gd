...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 51, "Registry indexes 51 production VN backgrounds across nine completed locations.")
	...
	_expect(artwork_phases.size() == 4 and artwork_asset_count == 52, "VN art provides four prioritized production phases covering 52 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 9, "Room-first production records nine fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 51, "Nine complete locations provide 51 unique ready base-day backgrounds.")
