...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 133, "Registry indexes 133 production VN backgrounds across twenty-four completed locations.")
	...
	_expect(artwork_phases.size() == 18 and artwork_asset_count == 133, "VN art provides eighteen prioritized production phases covering 133 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 24, "Room-first production records twenty-four fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 133, "Twenty-four complete locations provide 133 unique ready base-day backgrounds.")
