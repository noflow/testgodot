...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 95, "Registry indexes 95 production VN backgrounds across sixteen completed locations.")
	...
	_expect(artwork_phases.size() == 10 and artwork_asset_count == 95, "VN art provides ten prioritized production phases covering 95 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 16, "Room-first production records sixteen fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 95, "Sixteen complete locations provide 95 unique ready base-day backgrounds.")
