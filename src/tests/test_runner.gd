...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 349, "Registry indexes 349 production VN backgrounds across sixty-two completed locations.")
	...
	_expect(artwork_phases.size() == 56 and artwork_asset_count == 349, "VN art provides fifty-six prioritized production phases covering 349 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 62, "Room-first production records sixty-two fully completed base-art locations.")
