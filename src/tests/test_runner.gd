...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 286, "Registry indexes 286 production VN backgrounds across fifty-one completed locations.")
	...
	_expect(artwork_phases.size() == 45 and artwork_asset_count == 286, "VN art provides forty-five prioritized production phases covering 286 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 51, "Room-first production records fifty-one fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 286, "Fifty-one complete locations provide 286 unique ready base-day backgrounds.")
