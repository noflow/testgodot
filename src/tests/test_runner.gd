...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 86, "Registry indexes 86 production VN backgrounds across fifteen completed locations.")
	...
	_expect(artwork_phases.size() == 9 and artwork_asset_count == 86, "VN art provides nine prioritized production phases covering 86 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 15, "Room-first production records fifteen fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 86, "Fifteen complete locations provide 86 unique ready base-day backgrounds.")
