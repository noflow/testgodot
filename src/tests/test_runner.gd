...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 341, "Registry indexes 341 production VN backgrounds across sixty-one completed locations.")
	...
	_expect(artwork_phases.size() == 55 and artwork_asset_count == 341, "VN art provides fifty-five prioritized production phases covering 341 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 61, "Room-first production records sixty-one fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 341, "Sixty-one complete locations provide 341 unique ready base-day backgrounds.")
