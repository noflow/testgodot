...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 273, "Registry indexes 273 production VN backgrounds across forty-eight completed locations.")
	...
	_expect(artwork_phases.size() == 42 and artwork_asset_count == 273, "VN art provides forty-two prioritized production phases covering 273 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 48, "Room-first production records forty-eight fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 273, "Forty-eight complete locations provide 273 unique ready base-day backgrounds.")
