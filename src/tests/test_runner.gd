...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 131, "Registry indexes 131 production VN backgrounds across twenty-three completed locations.")
	...
	_expect(artwork_phases.size() == 17 and artwork_asset_count == 131, "VN art provides seventeen prioritized production phases covering 131 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 23, "Room-first production records twenty-three fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 131, "Twenty-three complete locations provide 131 unique ready base-day backgrounds.")
