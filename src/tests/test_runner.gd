...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 140, "Registry indexes 140 production VN backgrounds across twenty-six completed locations.")
	...
	_expect(artwork_phases.size() == 20 and artwork_asset_count == 140, "VN art provides twenty prioritized production phases covering 140 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 26, "Room-first production records twenty-six fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 140, "Twenty-six complete locations provide 140 unique ready base-day backgrounds.")
