...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 150, "Registry indexes 150 production VN backgrounds across twenty-eight completed locations.")
	...
	_expect(artwork_phases.size() == 22 and artwork_asset_count == 150, "VN art provides twenty-two prioritized production phases covering 150 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 28, "Room-first production records twenty-eight fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 150, "Twenty-eight complete locations provide 150 unique ready base-day backgrounds.")
