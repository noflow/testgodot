...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 99, "Registry indexes 99 production VN backgrounds across seventeen completed locations.")
	...
	_expect(artwork_phases.size() == 11 and artwork_asset_count == 99, "VN art provides eleven prioritized production phases covering 99 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 17, "Room-first production records seventeen fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 99, "Seventeen complete locations provide 99 unique ready base-day backgrounds.")
