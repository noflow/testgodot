...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 57, "Registry indexes 57 production VN backgrounds across eleven completed locations.")
	...
	_expect(artwork_phases.size() == 5 and artwork_asset_count == 57, "VN art provides five prioritized production phases covering 57 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 11, "Room-first production records eleven fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 57, "Eleven complete locations provide 57 unique ready base-day backgrounds.")
