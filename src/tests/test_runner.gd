...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 192, "Registry indexes 192 production VN backgrounds across thirty-five completed locations.")
	...
	_expect(artwork_phases.size() == 29 and artwork_asset_count == 192, "VN art provides twenty-nine prioritized production phases covering 192 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 35, "Room-first production records thirty-five fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 192, "Thirty-five complete locations provide 192 unique ready base-day backgrounds.")
