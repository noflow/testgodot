...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 186, "Registry indexes 186 production VN backgrounds across thirty-four completed locations.")
	...
	_expect(artwork_phases.size() == 28 and artwork_asset_count == 186, "VN art provides twenty-eight prioritized production phases covering 186 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 34, "Room-first production records thirty-four fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 186, "Thirty-four complete locations provide 186 unique ready base-day backgrounds.")
