...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 259, "Registry indexes 259 production VN backgrounds across forty-five completed locations.")
	...
	_expect(artwork_phases.size() == 39 and artwork_asset_count == 259, "VN art provides thirty-nine prioritized production phases covering 259 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 45, "Room-first production records forty-five fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 259, "Forty-five complete locations provide 259 unique ready base-day backgrounds.")
