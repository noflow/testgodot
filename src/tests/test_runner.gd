...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 243, "Registry indexes 243 production VN backgrounds across forty-three completed locations.")
	...
	_expect(artwork_phases.size() == 37 and artwork_asset_count == 243, "VN art provides thirty-seven prioritized production phases covering 243 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 43, "Room-first production records forty-three fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 243, "Forty-three complete locations provide 243 unique ready base-day backgrounds.")
