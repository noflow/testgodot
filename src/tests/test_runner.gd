...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 371, "Registry indexes 371 production VN backgrounds across sixty-five completed locations.")
	...
	_expect(artwork_phases.size() == 59 and artwork_asset_count == 371, "VN art provides fifty-nine prioritized production phases covering 371 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 65, "Room-first production records sixty-five fully completed base-art locations.")
