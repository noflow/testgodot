...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 41, "Registry indexes 41 production VN backgrounds across eight completed locations.")
	...
	_expect(artwork_phases.size() == 4 and artwork_asset_count == 46, "VN art provides four prioritized production phases covering 46 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 8, "Room-first production records eight fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 41, "Eight complete locations provide 41 unique ready base-day backgrounds.")
