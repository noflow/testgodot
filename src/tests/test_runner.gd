...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 324, "Registry indexes 324 production VN backgrounds across fifty-eight completed locations.")
	...
	_expect(artwork_phases.size() == 52 and artwork_asset_count == 324, "VN art provides fifty-two prioritized production phases covering 324 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 58, "Room-first production records fifty-eight fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 324, "Fifty-eight complete locations provide 324 unique ready base-day backgrounds.")
