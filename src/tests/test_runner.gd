...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 303, "Registry indexes 303 production VN backgrounds across fifty-three completed locations.")
	...
	_expect(artwork_phases.size() == 47 and artwork_asset_count == 303, "VN art provides forty-seven prioritized production phases covering 303 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 53, "Room-first production records fifty-three fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 303, "Fifty-three complete locations provide 303 unique ready base-day backgrounds.")
