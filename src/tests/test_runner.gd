...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 26, "Registry indexes 26 VN background assignments across the completed opening locations and park path.")
	...
	_expect(artwork_phases.size() == 4 and artwork_asset_count == 35, "VN art provides four prioritized production phases covering 35 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 5, "Room-first production records five fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 25, "Five complete opening locations provide 25 unique ready base-day backgrounds.")
