...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 80, "Registry indexes 80 production VN backgrounds across fourteen completed locations.")
	...
	_expect(artwork_phases.size() == 8 and artwork_asset_count == 80, "VN art provides eight prioritized production phases covering 80 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 14, "Room-first production records fourteen fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 80, "Fourteen complete locations provide 80 unique ready base-day backgrounds.")
