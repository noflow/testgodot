...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 308, "Registry indexes 308 production VN backgrounds across fifty-four completed locations.")
	...
	_expect(artwork_phases.size() == 48 and artwork_asset_count == 308, "VN art provides forty-eight prioritized production phases covering 308 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 54, "Room-first production records fifty-four fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 308, "Fifty-four complete locations provide 308 unique ready base-day backgrounds.")
