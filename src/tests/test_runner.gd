...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 317, "Registry indexes 317 production VN backgrounds across fifty-six completed locations.")
	...
	_expect(artwork_phases.size() == 50 and artwork_asset_count == 317, "VN art provides fifty prioritized production phases covering 317 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 56, "Room-first production records fifty-six fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 317, "Fifty-six complete locations provide 317 unique ready base-day backgrounds.")
