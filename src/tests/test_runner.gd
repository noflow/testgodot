...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 330, "Registry indexes 330 production VN backgrounds across fifty-nine completed locations.")
	...
	_expect(artwork_phases.size() == 53 and artwork_asset_count == 330, "VN art provides fifty-three prioritized production phases covering 330 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 59, "Room-first production records fifty-nine fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 330, "Fifty-nine complete locations provide 330 unique ready base-day backgrounds.")
