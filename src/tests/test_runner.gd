...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 117, "Registry indexes 117 production VN backgrounds across twenty-one completed locations.")
	...
	_expect(artwork_phases.size() == 15 and artwork_asset_count == 117, "VN art provides fifteen prioritized production phases covering 117 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 21, "Room-first production records twenty-one fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 117, "Twenty-one complete locations provide 117 unique ready base-day backgrounds.")
