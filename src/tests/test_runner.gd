...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 208, "Registry indexes 208 production VN backgrounds across thirty-eight completed locations.")
	...
	_expect(artwork_phases.size() == 32 and artwork_asset_count == 208, "VN art provides thirty-two prioritized production phases covering 208 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 38, "Room-first production records thirty-eight fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 208, "Thirty-eight complete locations provide 208 unique ready base-day backgrounds.")
