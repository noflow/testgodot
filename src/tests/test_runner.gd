...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 74, "Registry indexes 74 production VN backgrounds across thirteen completed locations.")
	...
	_expect(artwork_phases.size() == 7 and artwork_asset_count == 74, "VN art provides seven prioritized production phases covering 74 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 13, "Room-first production records thirteen fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 74, "Thirteen complete locations provide 74 unique ready base-day backgrounds.")
