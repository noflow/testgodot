...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 54, "Registry indexes 54 production VN backgrounds across ten completed locations.")
	...
	_expect(artwork_phases.size() == 4 and artwork_asset_count == 54, "VN art provides four prioritized production phases covering 54 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 10, "Room-first production records ten fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 54, "Ten complete locations provide 54 unique ready base-day backgrounds.")
