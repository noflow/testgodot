...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 203, "Registry indexes 203 production VN backgrounds across thirty-seven completed locations.")
	...
	_expect(artwork_phases.size() == 31 and artwork_asset_count == 203, "VN art provides thirty-one prioritized production phases covering 203 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 37, "Room-first production records thirty-seven fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 203, "Thirty-seven complete locations provide 203 unique ready base-day backgrounds.")
