...
func _test_content_registry() -> void:
	...
	_expect(_registry.get_all("vn_backgrounds").size() == 137, "Registry indexes 137 production VN backgrounds across twenty-five completed locations.")
	...
	_expect(artwork_phases.size() == 19 and artwork_asset_count == 137, "VN art provides nineteen prioritized production phases covering 137 room backgrounds.")
	...
	_expect(artwork_backlog.get("room_scope", {}).get("completed_locations", []).size() == 25, "Room-first production records twenty-five fully completed base-art locations.")
	...
	_expect(production_base_backgrounds == 137, "Twenty-five complete locations provide 137 unique ready base-day backgrounds.")
